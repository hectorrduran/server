...
internals.Generator.prototype.request = function (connection, req, res, options) {
    ...
    if (this._decorations) {
        var properties = Object.keys(this._decorations);
        for (let i = 0; i < properties.length; ++i) {
            var property = properties[i];
            var assignment = this._decorations[property];
...
internals.Request = function (connection, req, res, options) {
    ...
    this._log(['received'], about, now);     // Must be last for object to be fully varructed
};
...
internals.Request.prototype._setTimeouts = function () {
    ...
    if (serverTimeout) {
        serverTimeout = Math.floor(serverTimeout - this._bench.elapsed());          // Calculate the timeout from when the request was varructed
        var timeoutReply = () => {
...
internals.Request.prototype._invoke = function (event, callback) {
    ...
    this._protect.run(callback, (exit) => {
        ...
        var each = (ext, next) => {
            ...
            var options = { postHandler: (event.type === 'onPostHandler' || event.type === 'onPreResponse') };
            var reply = this.server._replier.interface(this, ext.plugin.realm, options, finalize);
