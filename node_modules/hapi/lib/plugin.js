...
internals.Plugin.prototype.register = function (plugins /*, [options], callback */) {
    ...
    */
    ...
    var each = (item, next) => {
        ...
        var selection = this._select(item.options.select, item.name);
        ...
        var connectionless = (item.connections === 'conditional' ? selection.connections.length === 0 : !item.connections);
        if (connectionless) {
...
internals.cache = (plugin) => {
    ...
    var policy = function (options, _segment) {

        options = Schema.apply('cachePolicy', options);

        var segment = options.segment || _segment || (plugin.realm.plugin ? '!' + plugin.realm.plugin : '');
        Hoek.assert(segment, 'Missing cache segment name');

        var cacheName = options.cache || '_default';
        var cache = plugin.root._caches[cacheName];
...
internals.Plugin.prototype.log = function (tags, data, timestamp, _internal) {
    ...
    var update = (typeof data !== 'function' ? { timestamp, tags, data, internal } : () => {
