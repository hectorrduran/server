...
internals.Auth.prototype.test = function (name, request, next) {
    ...
    var reply = request.server._replier.interface(request, strategy.realm, { data: true }, (response) => next(response, reply._data && reply._data.credentials));
...
internals.setupScope = function (access) {
    ...
    for (let i = 0; i < access.scope.length; ++i) {
        var value = access.scope[i];
        var prefix = value[0];
        var type = (prefix === '+' ? 'required' : (prefix === '!' ? 'forbidden' : 'selection'));
        var clean = (type === 'selection' ? value : value.slice(1));
...
internals.Auth.prototype._authenticate = function (request, next) {
    ...
    var authenticator = new internals.Authenticator(config, request, this);
...
internals.Authenticator = class {
    varructor(config, request, manager) {
    ...
    execute(next) {
        ...
        if (this.current < config.strategies.length) {
            var name = config.strategies[this.current];
            var after = (err, data) => this.validate(err, data, next);
            request._protect.run(after, (exit) => {

                var strategy = this.manager._strategies[name];
                var reply = request.server._replier.interface(request, strategy.realm, { data: true }, (err) => exit(err, reply._data));
                strategy.methods.authenticate(request, reply);
            });
        ...
        var err = Boom.unauthorized('Missing authentication', this.errors);
    ...
    validate(err, result, next) {                 // err can be Boom, Error, or a valid response object
        ...
        var config = this.config;
        var request = this.request;
        var name = config.strategies[this.current] || 'bypass';
        ...
        var error = internals.access(request, config, credentials, name);
        if (!error) {
...
internals.access = function (request, config, credentials, name) {
    ...
    var requestEntity = (credentials.user ? 'user' : 'app');

    var scopeErrors = [];
    for (let i = 0; i < config.access.length; ++i) {
        var access = config.access[i];
    ...
    if (scopeErrors.length) {
        var data = { got: credentials.scope, need: scopeErrors };
...
internals.validateScope = function (credentials, scope, type) {
    ...
    var count = typeof credentials.scope === 'string' ?
