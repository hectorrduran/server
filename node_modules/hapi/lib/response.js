...
internals.Response.prototype.etag = function (tag, options) {
    ...
    var entity = internals.Response.entity(tag, options);
...
internals.Response.unmodified = function (request, options) {
    ...
    if (options.etag &&
        ...
        for (let i = 0; i < ifNoneMatch.length; ++i) {
            ...
            if (options.vary) {
                var etagBase = options.etag.slice(0, -1);
                var encoders = request.connection._compression.encodings;
    ...
    if (ifModifiedSinceHeader &&
        ...
        var lastModified = internals.parseDate(options.modified);
...
internals.Response.prototype._prepare = function (next) {
    ...
    var onDone = Hoek.nextTick((source) => {
    ...
    var onError = (source) => {
        ...
        if (!(source instanceof Error)) {
            var err = new Error('Rejected promise');
            err.data = source;
...
internals.Response.prototype._streamify = function (source, next) {
    ...
    if (this.variety === 'plain' &&
        ...
        var escape = this.request.route.settings.json.escape || false;
        try {
            if (replacer || space) {
