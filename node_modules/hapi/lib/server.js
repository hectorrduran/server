...
exports = module.exports = internals.Server = function (options) {
    ...
    if (this._settings.debug) {
        ...
        if (this._settings.debug.log) {
            var filter = this._settings.debug.log.some((tag) => tag === '*') ? undefined : this._settings.debug.log;
            this._events.on({ name: 'log', filter }, (event) => debug(null, event));
        }

        if (this._settings.debug.request) {
            var filter = this._settings.debug.request.some((tag) => tag === '*') ? undefined : this._settings.debug.request;
...
internals.Server.prototype._createCache = function (options, _callback) {
    ...
    for (let i = 0; i < options.length; ++i) {
        ...
        if (typeof config.engine === 'object') {
            ...
            var settings = Hoek.clone(config);
    ...
    if (['initialized', 'starting', 'started'].indexOf(this._state) !== -1) {
        var each = (client, next) => client.start(next);
...
internals.Server.prototype.start = function (callback) {
    ...
    var nextTickCallback = Hoek.nextTick(callback);
    ...
    if (this._state === 'started') {
        var each = (connection, next) => connection._start(next);
...
internals.Server.prototype.initialize = function (callback) {
    ...
    var caches = Object.keys(this._caches);
    var each = (cache, next) => this._caches[cache].client.start(next);
...
internals.Server.prototype.stop = function (/* [options], callback */) {
    ...
    var lastArg = arguments[args - 1];
    var callback = (!args ? null : (typeof lastArg === 'function' ? lastArg : null));
    var options = (!args ? {} : (args === 1 ? (callback ? {} : arguments[0]) : arguments[0]));
    ...
    this._invoke('onPreStop', (err) => {
        ...
        var each = (connection, next) => connection._stop(options, next);
        Items.parallel(this.connections, each, (err) => {
