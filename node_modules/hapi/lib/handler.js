...
internals.prerequisites = function (request, callback) {
    ...
    var domain = request.domain;      // Save a reference to the current domain
    ...
    Items.serial(request._route._prerequisites, each, (err) => {
        ...
        var wrapped = domain ? domain.bind(internals.handler) : internals.handler;
...
internals.handler = function (request, callback) {
    ...
    var result = request.route.settings.handler.call(bind, request, reply);
    if (result &&
...
internals.fromString = function (type, notation, server) {
    ...
    result.method = (request, reply) => {
        ...
        var finalize = (err, value, cached, report) => {
