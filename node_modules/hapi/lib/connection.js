...
internals.Connection.prototype._init = function () {
    ...
    this.listener.once('listening', () => {
        ...
        if (this.type === 'tcp') {
            var address = this.listener.address();
...
internals.Connection.prototype._dispatch = function (options) {
    ...
    return (req, res) => {
        ...
        var request = this.server._requestor.request(this, req, res, options);

        // Check load

        var overload = this._load.check();
        if (overload) {
...
internals.Connection.prototype._addRoute = function (config, plugin) {
    ...
    for (let i = 0; i < vhosts.length; ++i) {
        var vhost = vhosts[i];
        var record = this._router.add({ method: route.method, path: route.path, vhost, analysis: route._analysis, id: route.settings.id }, route);
